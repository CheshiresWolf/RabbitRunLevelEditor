require(["jquery", "contextMenu_min"], function($, contextMenu) {
	...
	function printJson(name) {
		var res = {
			name : (name) ? name : "fall000.json",
			base_path : "img/"
		};

		res.images = {
	        "box" : ["stone.png", "stone2.png", "stone3.png", "koloda.png", "mushroom1.png", "mushroom2.png", "tukva.png"],
		    "trap" : [{
	            "file"   : "box_trap.xml",
	            "frames" : "6",
	            "speed"  : "100",
		...
		var levelJSON = IBuilMyOwnStringify(res);
		print(levelJSON);
	...
	document.getElementById("fileInput").addEventListener('change', function(e) {
		...
		reader.onload = function(e) {
			var bufLevel = JSON.parse(this.result);

			traps   = (bufLevel.scheme.traps) ? bufLevel.scheme.traps : [];
			boxes   = (bufLevel.scheme.boxes) ? bufLevel.scheme.boxes : [];
			carrots = (bufLevel.scheme.carrot) ? bufLevel.scheme.carrot : [];
			hole    = (bufLevel.scheme.hole) ? bufLevel.scheme.hole : null;
			rabbit  = (bufLevel.scheme.rabbit) ? bufLevel.scheme.rabbit : null;
			if (bufLevel.scheme.lock) {
				doors   = (bufLevel.scheme.lock.doors) ? bufLevel.scheme.lock.doors : [];
				spiderKeys = (bufLevel.scheme.lock.keys) ? bufLevel.scheme.lock.keys : [];
			}
